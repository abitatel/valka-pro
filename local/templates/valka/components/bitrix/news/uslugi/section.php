...
$res = CIBlockSection::GetList(
	[],
	["IBLOCK_ID" => "3", 'CODE' => $arResult["VARIABLES"]["SECTION_CODE"]],
	false,
	['*', 'UF_*']
);
...
$resForSEO = CIBlockSection::GetList(array(), array('IBLOCK_ID' => $arParams["IBLOCK_ID"], 'CODE' => $arResult["VARIABLES"]["SECTION_CODE"]));
$sectionForSEO = $resForSEO->Fetch();

$ipropElementValues = new \Bitrix\Iblock\InheritedProperty\SectionValues($arParams['IBLOCK_ID'], $sectionForSEO["ID"]);
$pageProperties = $ipropElementValues->getValues();

if(!empty($pageProperties['SECTION_META_TITLE'])) {
	$APPLICATION->SetPageProperty('title', $pageProperties['SECTION_META_TITLE']);
} else {
	$APPLICATION->SetPageProperty('title', $sectionData["NAME"]);
}
	
if(!empty($pageProperties['SECTION_META_DESCRIPTION']))
	$APPLICATION->SetPageProperty('description', $pageProperties['SECTION_META_DESCRIPTION']);

if(!empty($pageProperties['SECTION_META_KEYWORDS']))
	$APPLICATION->SetPageProperty('keywords', $pageProperties['SECTION_META_KEYWORDS']);
?>
<div class="max s-bar">
	...
	<div class="right-block">
		<div class="banner-page" style="background-image: url(<?= ($sectionData["PICTURE"]) ? CFile::GetPath($sectionData["PICTURE"]) : "/local/assets/img/pages/uslugi.png"; ?>);">
			<h1 class="p-title"><span><?= $sectionData["NAME"]; ?></span></h1>
			<p class="txt"><?= $sectionData["UF_SHORT"]; ?></p>
		</div>
